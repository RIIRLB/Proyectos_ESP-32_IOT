# ============================================================
#  PIF_NODE v11.8 — Plantilla Universal / LAB-ARTE
#
...
def escanear_canal():
    ...
    try:
        _sta.config(channel=_canal_actual)
        utime.sleep_ms(150)
        print("[CH] Forzado a", _canal_actual)
    except Exception as e:
        print("[CH] config fallo:", e)
    return False
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v11.8",     cx(font_sm, "v11.8"),     92, GRIS)
    backlight.value(1)
    utime.sleep_ms(2000)
    backlight.value(0)
...
def enviar_fb(en, payload, parent_id, mid_origen=None):
    # Verificar canal antes de enviar
    try:
        ch = _sta.config('channel')
        if ch != _canal_actual:
            _sta.config(channel=_canal_actual)
            utime.sleep_ms(50)
    except: pass
    ...
    try:
        ...
        # CRÍTICO: dar tiempo a la cola interna de ESP-NOW para procesar
        # el envío antes de aceptar otro. Sin esto: ESP_ERR_ESPNOW_NO_MEM.
        utime.sleep_ms(80)
        ...
        print("[FB ERR]", e)
        utime.sleep_ms(100)   # pausa larga tras error para liberar cola
        return False
...
def relay_fb_hijo(en, raw_str):
    try:
        ...
        if len(relay) < 248:
            ...
            utime.sleep_ms(80)   # mismo motivo que enviar_fb
            marca = "OK" if ok else "FALLO"
...
def ciclo(forzar=False):
    ...
    # ── Paso 3: Escuchar WAVE y relayar FBs ajenos ──────
    # En esta ventana también capturamos FBs de OTROS slaves que hagan
    # broadcast autónomo y hacemos relay hacia el Master. Sin esto,
    # los nietos (slaves fuera de rango directo) nunca llegan al Master.
    ...
    while utime.ticks_diff(fin, utime.ticks_ms()) > 0:
        ...
        try:
            ...
            if tipo_msg == "WAVE":
                ...
                wave_ttl      = ttl
                wave_mid      = data.get("mid")
                wave_recibida = True
                _conectado    = True
                _ultimo_padre = parent_id
                _ciclos_sin_wave = 0
                print("[<<< WAVE RX] de:{}  cmd:{}  target:{}  ttl:{}  mid:{}".format(
                    parent_id, wave_cmd, target, ttl, wave_mid))
                del data; break

            # FB de otro slave durante la ventana de escucha
            elif tipo_msg == "FB":
                ajeno_id = data.get("id", "?")
                ajeno_via = data.get("via", [])
                # Filtros: no es mío, no estoy en via, no relayado ya
                if ajeno_id == NODE_ID:
    ...
    if wave_ttl > 1:
        ...
        if wave_mid is not None:
            wave_hijos_dict["mid"] = wave_mid
        wave_hijos = json.dumps(wave_hijos_dict)
        _barra_status("Propagando ttl:{}".format(wave_ttl - 1), AMARILLO)
        ok1 = en.send(BROADCAST_MAC, wave_hijos)
        utime.sleep_ms(150)
        ok2 = en.send(BROADCAST_MAC, wave_hijos)
        utime.sleep_ms(80)
        del wave_hijos
        print("[>>> WAVE PROP] mid:{}  ttl:{}  hw1:{}  hw2:{}".format(
            wave_mid, wave_ttl - 1, "OK" if ok1 else "FALLO", "OK" if ok2 else "FALLO"))

    gc.collect()

    # ── Paso 6: Escuchar FBs ajenos y hacer relay ──
    # CAMBIO IMPORTANTE: relayar TODO FB que no sea nuestro y no estemos
    # en su lista 'via'. Antes solo relayábamos FBs cuyo 'par' fuera NODE_ID,
    # lo que descartaba a los nietos (SLAVE_04 reportando MASTER como par).
    ...
    while utime.ticks_diff(fin_hijos, utime.ticks_ms()) > 0:
        ...
        try:
            ...
            if data.get("type") == "FB":
                ...
                via      = data.get("via", [])
                # No relayar si es propio, ya estamos en via, o ya lo relayamos
                if hijo_id == NODE_ID:
                    del data, txt; continue
                if NODE_ID in via:
                    del data, txt; continue
                ...
                # Track hijos directos (los que reportan parent==NODE_ID)
                # para el rol del nodo, pero el relay ocurre siempre.

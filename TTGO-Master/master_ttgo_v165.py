...
def encolar_status(ts):
    """
    Encola línea STATUS al servidor con la lista de nodos vistos.
    Formato: ts,MASTER,STATUS,SLAVE_01:directo:5s|SLAVE_02:via_SLAVE_01:7s|...
    """
...
# ───────────────────────────────────────────────
_msg_counter = 0
_ultimo_cmd_procesado = None   # (cmd, ts) del último comando procesado, para deduplicar retain
...
def _mqtt_cb(topic, msg):
    global _ultimo_cmd_procesado
    txt = msg.decode().strip()
    if not txt:
        return
    # Deduplicar comandos retenidos: si el mismo comando llega 2 ciclos
    # seguidos (porque sigue retenido en el broker), solo procesar el primero.
    # Después de 5 segundos consideramos que es un comando NUEVO con mismo texto.
    ahora = utime.ticks_ms()
    if _ultimo_cmd_procesado is not None:
        ult_txt, ult_ts = _ultimo_cmd_procesado
        if ult_txt == txt and utime.ticks_diff(ahora, ult_ts) < 30_000:
            print("[MQTT RX dedup]", txt, "(retain repetido, ignorando)")
            return
    _ultimo_cmd_procesado = (txt, ahora)
        ...
        try:
            ...
            if data.get("type") in ("FEEDBACK", "FB"):
                ...
                if nodo in nodos_procesados_esta_ventana:
                    ruta_dup = "via " + ",".join(via) if via else "directo"
                    print("[DEDUP] FB duplicado de {} ({})".format(nodo, ruta_dup))
                    del data, txt
                    continue
                ...
                # Actualizar dict con metadatos completos
...
def loop():
    ...
    while True:
        ...
        # Si WiFi no conectó, NO hacer mesh este ciclo — sta.config(channel=...)
        # con WiFi en estado intermedio causa abort() en core 1 (no atrapable).
        # Mejor esperar al siguiente ciclo donde WiFi puede recuperarse.
        if _wif_ok is False:
            print("[CICLO] WiFi falló — saltando mesh, reintentando en 5s")
            machine.lightsleep(5_000)
            t_ultimo_hb = utime.ticks_ms()
            continue

...
#
#  PROBLEMA: network.WLAN(STA_IF) llama internamente
#  esp_wifi_init(). En soft-reboot el driver C sigue vivo
#  → esp_wifi_init() retorna 0x0101 → RuntimeError.
#  Si esto está a nivel de módulo sin try/except → crash total
#  → nuevo soft reboot → loop infinito de crashes.
#
#  SOLUCIÓN:
#    1. Init envuelto en try/except → si explota, espera y reintenta
#    2. Fatal error usa machine.reset() (hard reset limpia driver C)
#       en vez de utime.sleep_ms (soft reboot no lo hace)
#
#  REGLA ABSOLUTA después de aquí:
#    ✅ _sta.connect() / _sta.disconnect()
#    ✅ _en.send() / _en.recv()
#    ❌ active(False) en cualquiera — NUNCA
# ───────────────────────────────────────────────
_sta = None
_en  = None
...
def _init_radio():
    """
    Inicializa WiFi y ESP-NOW de forma segura.
    Maneja el caso de soft-reboot donde el driver C ya está vivo.
    Retorna True si tuvo éxito.
    """
    global _sta, _en
    # network.WLAN() puede lanzar 0x0101 si el driver ya está vivo
    # tras un soft-reboot. Intentamos hasta 3 veces con pausa.
    for intento in range(3):
        try:
            ...
            print("[RADIO INIT] WiFi intento {}/3: {}".format(intento+1, e))
            utime.sleep_ms(1500)
    else:
        return False   # no se pudo init WiFi

    # ESP-NOW: OSError si ya está activo (soft-reboot) → ignorar
    try:
        _en = espnow.ESPNow()
        _en.active(True)
    except OSError:
    ...
    return _en is not None

# Intentar init al arrancar. Si falla → machine.reset() para hard reset
if not _init_radio():
    print("[RADIO] Init fallido — hard reset")
    utime.sleep_ms(2000)
    machine.reset()
...
while True:
    try:
        ...
        utime.sleep_ms(5000)
        backlight.value(0)
        # Hard reset: limpia el driver C de WiFi/ESP-NOW completamente.
        # Soft reboot (lo que haría un except simple) deja el driver
        # C vivo → la siguiente llamada a network.WLAN() lanza 0x0101.
        machine.reset()

# ============================================================
#  MASTER_TTGO v14.5 — PIF Mesh / LAB-ARTE
#  Flujo por ciclo:
...
def arranque_seguro():
    """
    Inicializar WiFi UNA sola vez. Nunca llamar sta.active(False) después.
    Principio v7.2: WiFi se activa al inicio y permanece activo TODA la sesión.
    ESP-NOW se monta encima temporalmente y se desmonta limpiamente.
    sta.active(False) escribe estado al NVS flash y corrompe arranques posteriores.
    """
    try:
        ...
        if not sta.active():
            sta.active(True)
            utime.sleep_ms(500)
        print("[RADIO] Listo — WiFi activo, canal por asignar")
        return True
...
def heartbeat():
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "Master v14.5",  4,   4,  VERDE)
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v14.5",        4,  68, AMARILLO)
...
def ventana_wifi():
    ...
    # WiFi ya está activo desde arranque_seguro() — no hacer active(False/True).
    # Solo conectar al AP si hace falta.
    sta = network.WLAN(network.STA_IF)
    if not sta.active():
        # Solo si algo lo apagó externamente (no debería pasar nunca)
        sta.active(True)
        utime.sleep_ms(300)
    ...
    if not _ntp_ok:
        try:
            ...
            _sock.setdefaulttimeout(5)   # máx 5s para DNS + UDP — evita hang infinito
            ...
            try: _sock.setdefaulttimeout(None)
            except: pass
    ...
    # v7.2: nunca active(False) antes de ESP-NOW.
    # WiFi está activo con sus 10 buffers — ESP-NOW se monta encima.
    # Solo fijar el canal correcto.
    try: sta.disconnect()
    except: pass
    utime.sleep_ms(200)
...
#  VENTANA MESH
#
#  WiFi ya está ON (nunca se apaga).
#  Solo fijar canal → init ESP-NOW → send/recv → deinit ESP-NOW.
#  NO hacer sta.active(False/True) aquí bajo ningún concepto.
# ───────────────────────────────────────────────
def ventana_mesh(cmd="REQ:ALL", target="ALL"):
    global _nodos_vistos

    sta = network.WLAN(network.STA_IF)
    # WiFi ya está activo — solo asegurar canal correcto
    try:
...
def loop():
    ...
    while True:
        ...
        # ── Botones PRIMERO — antes de cualquier operación lenta ──────
        # El ISR ya encendió el backlight. Aquí mostramos qué va a hacer
        # y ejecutamos la acción sin reset_radio() extra.

        if _flag_server:
            _flag_server = False
            # Feedback visual inmediato — backlight ya está ON del ISR
            tft.fill(NEGRO)
            tft.write(font_sm, "PIF MASTER",   4,  4, VERDE)
            tft.write(font_md, "Servidor",      4, 30, AMARILLO)
            tft.write(font_sm, "conectando...", 4, 70, GRIS)
            # Encolar medición actual (valores de la última lectura)
            ts = hora_local(con_fecha=True)
            cola_subida.append("{},{},T:{} H:{},sensor".format(
                ts, CLIENT_ID, _t_propio, _h_propio))
            ventana_wifi()        # sin reset_radio() antes ni después
            medir_propio()        # actualizar valores para la pantalla
            pantalla_completa()
        ...
        if _flag_mesh:
            ...
            tft.fill(NEGRO)
            tft.write(font_sm, "PIF MASTER", 4,  4, VERDE)
            tft.write(font_md, "Malla",       4, 30, AMARILLO)
            tft.write(font_sm, "WAVE...",     4, 70, GRIS)
            ventana_mesh(cmd="REQ:ALL", target="ALL")
            reset_radio()
            t_ultimo_hb = utime.ticks_ms()
            continue

        # ── Medir DESPUÉS de revisar botones ──────────────────────────
        medir_propio()
...
if not arranque_seguro():
    tft.fill(NEGRO)
    tft.write(font_md, "RADIO ERROR",    4,  8,  ROJO)
    tft.write(font_sm, "Desconectar USB",    4,  50, AMARILLO)
    tft.write(font_sm, "Power Cycle (USB)",  4,  74, AMARILLO)
    backlight.value(1)
    # En este punto el radio está en estado irrecuperable desde Python.
    # Solo un power cycle (desconectar USB) lo resuelve.
    # Mantener pantalla encendida para que el usuario vea el mensaje.
    while True:
        utime.sleep_ms(5000)

print("[OK] Radio listo — iniciando loop v14.5")
...
while True:
    try:
        ...
        utime.sleep_ms(3000)
        backlight.value(0)
        # soft reset — seguro porque NUNCA llegamos aquí con ESP-NOW activo
        # (ventana_mesh siempre hace en.active(False) antes de retornar)
        machine.reset()

# ============================================================
#  MASTER_TTGO v16.0 — PIF Mesh / LAB-ARTE
# ============================================================
...
def ui_status(linea1, linea2="", col1=AMARILLO, col2=GRIS):
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "PIF MASTER v16.0", 4, 4,  VERDE)
...
def heartbeat():
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "Master v16.0",  4,   4,  VERDE)
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v16.0",        4,  68, AMARILLO)
...
def ventana_mesh(cmd="REQ:ALL", target="ALL"):
    ...
    if cola_bajada:
        ...
        # Extraer cmd para mostrarlo en display
        try:
            d = json.loads(onda)
            cmd_mostrar = d.get("cmd", "WAVE")
        except: pass
        print("[MESH TX servidor]", onda[:60])
        # Feedback visual: una onda manda el servidor
    ...
    while utime.ticks_diff(fin, utime.ticks_ms()) > 0:
        ...
        try:
            ...
            if data.get("type") in ("FEEDBACK", "FB"):
                nodo    = data.get("id", "?")
                # Si ya procesamos este nodo en esta ventana, ignorar duplicado
                if nodo in nodos_procesados_esta_ventana:
                    print("[DEDUP] Ignorando FB duplicado de", nodo)
                    del data, txt
                    continue

...
def ui_status(linea1, linea2="", col1=AMARILLO, col2=GRIS):
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "PIF MASTER v16.3", 4, 4,  VERDE)
...
def heartbeat():
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "Master v16.3",  4,   4,  VERDE)
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v16.3",        4,  68, AMARILLO)
...
def _mqtt_cb(topic, msg):
    ...
    # Ignorar mensajes vacíos (son los que mandamos para limpiar retain)
    if not txt:
        return
    ...
    for intento in range(2):
        try:
            ...
            if modo == "completo":
                ...
                _comandos_antes = len(cola_bajada)
                for poll_i in range(4):     # 4 polls × 150ms = 600ms
                    client.check_msg()
                    utime.sleep_ms(150)
                # Si recibimos algún comando retenido, limpiarlo del broker
                # publicando vacío con retain=True. Sin esto, el comando viejo
                # se procesaría una y otra vez en cada ciclo del Master.
                if len(cola_bajada) > _comandos_antes:
                    print("[MQTT] Limpiando retain ({} comandos procesados)".format(
                        len(cola_bajada) - _comandos_antes))
                    try:
                        # Publicar empty payload con retain=True → borra el retain
                        client.publish(TOPIC_SUB, b"", retain=True)
                    except Exception as e:
                        print("[MQTT] Limpieza retain falló:", e)
...
def loop():
    ...
    while True:
        ...
        if cola_subida:
            ...
            encolar_status(ts_status)   # adjuntar árbol actualizado
            ventana_wifi(modo="flush")
            reset_radio()

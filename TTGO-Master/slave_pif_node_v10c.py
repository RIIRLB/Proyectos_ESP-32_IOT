# ============================================================
#  PIF_NODE v1.0b — Plantilla Universal / LAB-ARTE
#
...
# ───────────────────────────────────────────────
#  RADIO (ESP-NOW) - REPARACIÓN DE EMERGENCIA
# ───────────────────────────────────────────────
import network, machine, utime

# PASO A: Pausa física total para dejar que el voltaje se estabilice
utime.sleep_ms(500) 

# PASO B: Definir _sta SIN activarlo. 
# Si esto falla, el reset de abajo limpiará la fragmentación.
try:
    _sta = network.WLAN(network.STA_IF)
    # NO hacer _sta.active(True) todavía.
except OSError:
    machine.reset()

def iniciar_espnow():
    # Solo activamos la radio cuando sea estrictamente necesario
    try:
        if not _sta.active():
            _sta.active(True)
        en = espnow.ESPNow()
        ...
        machine.reset() # Si falla al activar, reseteo físico.
...
def leer_sensores():
    ...
    # --- FIX DHT11 ---
    t, h = "Err", "Err"
    # 1. Pausa de estabilización: el DHT11 es lento
    utime.sleep_ms(500) 
    
    for intento in range(3): # Reintento si falla
        try:
            sensor_dht.measure()
            t = sensor_dht.temperature()
            h = sensor_dht.humidity()
            if t != 0 or h != 0: # Evitar lecturas falsas de ceros
                break
        except:
            utime.sleep_ms(200) # Espera un poco más si falló
    # -----------------
...
while True:
    try:
        ...
        # reseteamos para que la basura desaparezca.
        machine.reset()

# ============================================================
#  MASTER_TTGO v16.5 — PIF Mesh / LAB-ARTE
# ============================================================
...
_az_propio    = "--"
_estado_mov   = "---"
# Diccionario de nodos vistos:
#   _nodos_vistos[ID] = {
#     "ultimo_visto" : utime.ticks_ms(),
#     "parent"       : nodo padre reportado en el FB,
#     "count"        : nº de FBs recibidos,
#     "via_directo"  : True si vino sin relay (vacío campo "via"),
#     "via_relay"    : última lista via[]  si vino por relay,
#   }
_nodos_vistos = {}
_canal_actual = 6
_ntp_ok       = False
...
def encolar_status(ts):
    """
    Encola una línea STATUS al servidor con la lista de nodos vistos.
    Formato: ts,MASTER,STATUS,SLAVE_01:directo|SLAVE_02:via_SLAVE_01|...
    El servidor parsea ese formato para mostrar el árbol real de la malla.
    """
        ...
        try:
            ...
            if data.get("type") in ("FEEDBACK", "FB"):
                ...
                if nodo in nodos_procesados_esta_ventana:
                    print("[DEDUP] FB duplicado de", nodo,
                          "(via:{})".format(via) if via else "(directo)")
                    del data, txt
                    continue
                ...
                # Actualizar dict de nodos con info completa
                if nodo not in _nodos_vistos:
                    _nodos_vistos[nodo] = {"count": 0, "via_directo": False, "via_relay": []}
                info = _nodos_vistos[nodo]
                info["ultimo_visto"] = utime.ticks_ms()
                info["parent"]       = par
                info["count"]        = info.get("count", 0) + 1
                if via:
                    info["via_directo"] = False
                    info["via_relay"]   = via
                else:
                    info["via_directo"] = True
                    info["via_relay"]   = []
                # Limitar tamaño del dict (mantener los 15 más recientes)
...
def loop():
    ...
    while True:
        ...
        if cola_subida:
            ...
            # Agregar STATUS con árbol actualizado

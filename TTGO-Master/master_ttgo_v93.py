...
# ───────────────────────────────────────────────
# Init radio — safe para soft-reboot: el driver puede ya estar vivo
...
# ESPNow: si ya está activo (tras soft-reboot), lo obtenemos sin reiniciar
_en = espnow.ESPNow()
try:
    _en.active(True)
except OSError:
    pass   # ya estaba activo — OK, usamos el existente
try:
...
def loop():
    ...
    while True:
        ...
        if not _flag_server and not _flag_mesh:
            ...
            backlight.value(0)
            utime.sleep_ms(1000)   # NO lightsleep: apaga radio → 0x0101 al reiniciar
            backlight.value(1)

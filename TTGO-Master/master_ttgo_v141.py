# ============================================================
#  MASTER_TTGO v14.1 — PIF Mesh / LAB-ARTE
#
#  Fixes vs v13:
...
# ───────────────────────────────────────────────
def arranque_seguro():
    # Paso 1: cerrar ESP-NOW si quedó activo de sesión anterior
    try:
    ...
    # Paso 2: apagar el radio completamente y volver a encenderlo.
    # sta.active(False) → active(True) fuerza al driver IDF a liberar
    # los 4 rx-buffers de ESP-NOW y reservar los 10 de WiFi.
    try:
...
def ventana_mesh(cmd="REQ:ALL", target="ALL"):
    ...
    sta = network.WLAN(network.STA_IF)
    # Después de reset_radio (lightsleep), el driver puede haber bajado el radio.
    # sta.active(False) + active(True) garantiza estado limpio.
    if sta.active():
        sta.active(False)
        utime.sleep_ms(200)
    sta.active(True)
...
print("[OK] Radio listo — iniciando loop v14")

# ============================================================
#  MASTER_TTGO v8.2 — PIF Mesh / LAB-ARTE
#
#  Correcciones v8.0:
...
#  │  T: 23°C      H: 65%              │  y=24  font_md  ← GRANDE
#  │  [mensaje de estado]               │  y=76  font_sm
#  │  WiFi:OK  MQTT:OK  Svr:OK          │  y=108 font_sm (barra)
#  └────────────────────────────────────┘
# ───────────────────────────────────────────────
def _estado_str(val):
...
def ui_dash(msg="", col=None):
    ...
    # ── Fila 1: título pequeño + hora ──────────
    tft.write(font_sm, "PIF MASTER", 4,   2, VERDE)
    tft.write(font_sm, hora,         160, 2, CYAN)

    # ── Fila 2: Temperatura grande ─────────────
    t_txt = "T: {}C".format(estado["t"])
    tft.write(font_md, t_txt, 4, 24, AMARILLO)

    # ── Fila 3: Humedad grande ─────────────────
    h_txt = "H: {}%".format(estado["h"])
    tft.write(font_md, h_txt, 4, 54, CYAN)

    # ── Fila 4: Mensaje de estado ───────────────
    tft.write(font_sm, estado["msg"][:26], 4, 84, estado["msg_col"])

    # ── Fila 5: Barra WiFi / MQTT / Server ─────

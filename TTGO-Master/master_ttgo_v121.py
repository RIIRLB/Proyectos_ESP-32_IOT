...
T_RADIO_RESET  = 600      # lightsleep entre fases — reset de buffers de radio
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v12.1",        4,  68, AMARILLO)
...
def ventana_wifi():
    ...
    sta.active(True)
    # Sin sta.config(channel=6) en modo cliente:
    # el canal lo determina el AP durante la asociación.
    # Forzar canal 6 puede impedir conectar si el AP está en otro canal.
    utime.sleep_ms(500)   # Dar tiempo al radio para estabilizarse tras lightsleep
...
def ventana_mesh(cmd="REQ:ALL", target="ALL"):
    ...
    sta = network.WLAN(network.STA_IF)
    sta.active(True)
    sta.config(channel=6)   # Canal fijo para ESP-NOW — slaves usan el mismo canal
    utime.sleep_ms(200)     # Dar tiempo al radio para estabilizarse
...
def loop():
    ...
    # Reset de radio inicial: garantiza estado limpio tras soft reboot.
    # Sin esto, el primer ventana_wifi() corre con radio en estado indefinido.
    print("[INIT] Reset de radio inicial...")
    reset_radio()

# ============================================================
#  PIF_NODE v11.9 — Plantilla Universal / LAB-ARTE
#
...
def init_espnow(canal):
    ...
    try:
        ...
        print("[CH] config en init fallo:", e)
    en = espnow.ESPNow()
    en.active(True)
...
def cerrar_espnow(en):
    ...
    # NO apagar WiFi — mantenerlo activo siempre evita el bug
    # de buffers TX al reactivar (cola interna queda en estado raro).
    gc.collect()
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v11.9",     cx(font_sm, "v11.9"),     92, GRIS)
    backlight.value(1)
    utime.sleep_ms(2000)
    backlight.value(0)
...
def enviar_fb(en, payload, parent_id, mid_origen=None):
    if en is None:
        print("[FB ERR] en=None, init_espnow no fue llamado")
        return False
    pkt_dict = {
        "type": "FB",
        "id"  : NODE_ID,

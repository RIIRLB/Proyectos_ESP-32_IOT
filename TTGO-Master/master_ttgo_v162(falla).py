...
def ui_status(linea1, linea2="", col1=AMARILLO, col2=GRIS):
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "PIF MASTER v16.2", 4, 4,  VERDE)
...
def heartbeat():
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "Master v16.2",  4,   4,  VERDE)
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v16.2",        4,  68, AMARILLO)
...
def loop():
    ...
    while True:
        ...
        if _flag_server:
            ...
            cola_subida.append("{},{},T:{} H:{},sensor".format(
                ts, CLIENT_ID, _t_propio, _h_propio))
            ventana_wifi(modo="flush")   # rápido, sin polling
            medir_propio()
            pantalla_completa()

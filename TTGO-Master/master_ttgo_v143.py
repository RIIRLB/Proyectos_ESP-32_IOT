# ============================================================
#  MASTER_TTGO v14.3 — PIF Mesh / LAB-ARTE
#
#  Fixes vs v13:
...
# ───────────────────────────────────────────────
def deepsleep_seguro(ms):
    """
    SIEMPRE usar esta función en lugar de machine.deepsleep().
    En IDF v5.2.2, si ESP-NOW o WiFi están activos cuando entra en deepsleep,
    los 4 rx-buffers de ESP-NOW se preservan en memoria y sobreviven al wake.
    Al volver a arrancar, WiFi no puede conseguir sus 10 buffers → 0x0101 loop.
    Solución: deinit explícito de ESP-NOW y WiFi ANTES de dormir.
    """
    print("[SLEEP] Deinit radio antes de deepsleep...")
    try:
        en = espnow.ESPNow()
        en.active(False)
        del en
    ...
    gc.collect()
    utime.sleep_ms(200)
    print("[SLEEP] Entrando en deepsleep {}ms".format(ms))
    machine.deepsleep(ms)

def arranque_seguro():
    """
    NO hacer sta.active(False) aquí.
    Si WiFi nunca se inicializó, active(False) llama a esp_wifi_stop()
    sobre un driver no init → error 0x3001 que deja al driver en estado
    inválido → el siguiente active(True) también falla.
    Solo hacer active(True) y dejar que el IDF maneje el estado inicial.
    Si falla con 0x0101 (buffers ESP-NOW residuales): deepsleep_seguro limpia todo.
    """
    try:
        sta = network.WLAN(network.STA_IF)
        sta.active(True)
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v14.3",        4,  68, AMARILLO)
...
def ventana_mesh(cmd="REQ:ALL", target="ALL"):
    ...
    # NO hacer sta.active(False) aquí — v7.2 (funcional) nunca lo hacía.
    # Solo asegurar que el radio esté ON y fijar canal.
    sta = network.WLAN(network.STA_IF)
    if not sta.active():
        sta.active(True)
        utime.sleep_ms(300)
    ...
    print("[MESH] ESP-NOW init, canal", _canal_actual)
    try:
        en = espnow.ESPNow()
        en.active(True)
    except Exception as e:
        print("[ESP-NOW ERR]", e)
        deepsleep_seguro(2000)
...
if not arranque_seguro():
    ...
    utime.sleep_ms(2000)
    backlight.value(0)
    deepsleep_seguro(2000)   # deinit ESP-NOW + WiFi ANTES de dormir

print("[OK] Radio listo — iniciando loop v14.3")

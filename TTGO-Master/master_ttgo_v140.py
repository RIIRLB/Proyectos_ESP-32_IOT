# ============================================================
#  MASTER_TTGO v14.0 — PIF Mesh / LAB-ARTE
#
#  Fixes vs v13:
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v14.0",        4,  68, AMARILLO)
...
def ventana_wifi():
    ...
    _wif_ok = True
    _canal_actual = sta.config('channel')   # canal real que usó el AP
...
def ventana_mesh(cmd="REQ:ALL", target="ALL"):
    ...
    sta = network.WLAN(network.STA_IF)
    sta.active(True)                   # Radio ON tras lightsleep
    sta.config(channel=_canal_actual)  # Canal = el del AP actual
    utime.sleep_ms(200)

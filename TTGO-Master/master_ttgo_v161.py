# ============================================================
#  MASTER_TTGO v16.1 — PIF Mesh / LAB-ARTE
# ============================================================
...
def ui_status(linea1, linea2="", col1=AMARILLO, col2=GRIS):
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "PIF MASTER v16.1", 4, 4,  VERDE)
...
def heartbeat():
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "Master v16.1",  4,   4,  VERDE)
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v16.1",        4,  68, AMARILLO)
...
def ventana_wifi():
    ...
    client = None
    for intento in range(2):   # máx 2 intentos en caso de ECONNABORTED
        try:
            ...
            # Múltiples check_msg con espera para atrapar comandos pendientes
            # del servidor que llegaron al broker durante el sleep del Master.
            print("[MQTT] Buscando comandos pendientes...")
            for poll_i in range(6):     # 6 polls × 200ms = 1.2s de escucha
                client.check_msg()
                utime.sleep_ms(200)
            ...
            gc.collect()
            if intento == 0:
                utime.sleep_ms(1500)   # pausa antes de reintentar
    else:
        pass   # ambos intentos fallaron — continuar sin MQTT

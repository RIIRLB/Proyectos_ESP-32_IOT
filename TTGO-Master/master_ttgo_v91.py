...
# ───────────────────────────────────────────────
def _espnow_cerrar(en):
    """Cierra ESP-NOW de forma segura. Llamar siempre en finally."""
    try: en.active(False)
    except: pass
    try: del en
    except: pass
...
def ventana_mesh():
    ...
    # ── Iniciar ESP-NOW con recuperación de ESPNOW_EXIST ──
    # Si el driver quedó vivo de un ciclo anterior (por excepción),
    # obtenemos la instancia existente, la cerramos y la reabrimos.
    en = None
    try:
        en = espnow.ESPNow()
        en.active(True)
        ...
        if e.args[0] == -12395:   # ESP_ERR_ESPNOW_EXIST
            print("[MESH] ESPNow ya activo, reiniciando...")
            try:
                if en is None:
                    en = espnow.ESPNow()
                en.active(False)
    ...
    try:
        ...
        # WAVE a enviar (calculada una vez fuera del bucle)
        if cola_bajada:
        ...
        for _ in range(BROADCAST_N):
            try: en.send(BROADCAST_MAC, wave)
            except: pass
            utime.sleep_ms(150)
        del wave
        ...
        # SIEMPRE se ejecuta: libera el driver aunque haya excepción
        _espnow_cerrar(en)

...
#              Si pasan T_RESCAN_MS sin recibir ninguna WAVE, el nodo
#              vuelve a escanear ch 1, 6 y 11 buscando al master. Antes
#              se quedaba pegado para siempre en el último canal del
#              escaneo inicial si éste fallaba.
#    [NUEVO 2] Pulsación larga del botón izquierdo (3s) cambia el modo
#              entre siempre-despierto y lightsleep. La elección se
#              guarda en flash (node_config.json) y sobrevive reinicios.
#    [NUEVO 3] Manejo del comando "BEACON" del master: se sincroniza
#              canal/reloj y se propaga, pero no genera FB de respuesta.
#              (En realidad esto ya funciona porque target="NONE"
#              produce debe_responder=False; documentado para claridad.)
#
#  Cambios vs v11.7:
#    - MODO_SIEMPRE_DESPIERTO flag global para nodos siempre activos.
#    - Sincronización de RTC desde el campo "ts" del WAVE.
#    - Dedup global de WAVEs por mid con TTL.
# ============================================================
...
def ui_bienvenida():
    ...
    modo = "SIEMPRE ON" if MODO_SIEMPRE_DESPIERTO else "v11.9 sleep"
    ...
    while True:
        try:
            ...
            if utime.ticks_diff(utime.ticks_ms(), ultimo_wave_rx) > T_RESCAN_MS:
                print("[RESCAN] {}s sin WAVE, escaneando canales...".format(
                    T_RESCAN_MS // 1000))
                backlight.value(1)
                # Cerrar ESP-NOW global, escanear, reabrir en el nuevo canal
                try: en.active(False)
                except: pass
                escanear_canal()
                _sta.active(True)
                ...
                en.add_peer(BROADCAST_MAC)
                _en_global = en
                ultimo_wave_rx = utime.ticks_ms()   # reset contador
                backlight.value(0)
                continue

# ============================================================
#  MASTER_TTGO v15.2 — PIF Mesh / LAB-ARTE
#
...
import gc, network, machine, utime, json
...
def arranque_radio():
    ...
    # 1. WiFi primero — reserva sus 10 rx-buffers
    try:
    ...
    # 2. ESP-NOW — importar AQUÍ, después de WiFi activo.
    # Si se importa a nivel de módulo, el IDF preasigna 4 buffers
    # antes de que WiFi tenga sus 10 → error 0x0101 en arranque limpio.
    try:
        import espnow as _espnow_mod
        _en = _espnow_mod.ESPNow()
        _en.active(True)
        _en.add_peer(b'\xff\xff\xff\xff\xff\xff')
...
def ui_status(linea1, linea2="", col1=AMARILLO, col2=GRIS):
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "PIF MASTER v15.2", 4, 4,  VERDE)
...
def heartbeat():
    ...
    tft.fill(NEGRO)
    tft.write(font_sm, "Master v15.2",  4,   4,  VERDE)
...
def ui_bienvenida():
    ...
    tft.write(font_sm, "v15.2",        4,  68, AMARILLO)
...
def _mqtt_cb(topic, msg):
    ...
    print("[MQTT RX]", txt)
    # PAIR — el servidor pide confirmación de conexión
    # Respondemos publicando nuestra medición actual de inmediato
    if txt == "PAIR":
        ts = hora_local(con_fecha=True)
        cola_subida.append("{},{},T:{} H:{},sensor".format(
            ts, CLIENT_ID, _t_propio, _h_propio))
        print("[PAIR] Respondiendo al servidor")
        return
...
def ventana_wifi():
    ...
    # Desconectar del AP — el radio (WiFi driver) queda activo
    try: sta.disconnect()
    except: pass
    utime.sleep_ms(200)

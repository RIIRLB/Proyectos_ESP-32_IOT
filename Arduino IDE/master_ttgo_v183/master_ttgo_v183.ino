// ============================================================
//  MASTER_TTGO v18.3 — PIF Mesh / LAB-ARTE
//  Arduino C++ port — TTGO T-Display
//
//  Cambios vs v18.2:
//    [NUEVO 1] WAVE-beacon automático cada T_BEACON ms con target="NONE".
//              Los slaves usan el beacon para sincronizar canal y reloj
//              sin necesidad de un comando del servidor. Sin esto, un
//              slave que arranca en momento de silencio se queda pegado
//              en el último canal que escaneó (típicamente 11) y nunca
//              encuentra al master.
//    [NUEVO 2] Heartbeat STATUS al broker MQTT cada T_HB_MQTT ms.
//              Resuelve el "sin señal en Ns" del servidor cuando no hay
//              actividad de botones ni comandos.
//
//  Cambios vs v18.1:
...
const unsigned long T_BEACON       = 60000;  // [NUEVO 1] WAVE-beacon cada 60s
const unsigned long T_HB_MQTT      = 60000;  // [NUEVO 2] heartbeat MQTT cada 60s
...
unsigned long ultimoBeacon = 0;     // [NUEVO 1]
unsigned long ultimoHbMqtt = 0;     // [NUEVO 2]
...
// ───────────────────────────────────────────────
//  ENVIAR BEACON — WAVE periódica solo para sincronizar slaves
...
void enviarBeacon() {
  ...
  Serial.printf("[BEACON TX] ch:%d ts:%s\n",
...
void loop() {
  ...
  // [NUEVO 1] WAVE-beacon cada T_BEACON ms para que los slaves
  // sincronicen canal y reloj sin necesidad de comando del servidor.
  if (millis() - ultimoBeacon > T_BEACON) {
    ultimoBeacon = millis();
    enviarBeacon();
  }

  // [NUEVO 2] Heartbeat MQTT cada T_HB_MQTT ms para que el servidor
  // sepa que el master sigue vivo aunque no haya comandos ni botones.

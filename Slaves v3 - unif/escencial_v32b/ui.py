...
def _barra(msg, col=VERDE):
    tft.fill_rect(0, 108, W, H - 108, NEGRO)
    tft.fill_rect(4, 113, 8, 14, col)          # cuadrito indicador (como v12.4)
    tft.write(sm, msg[:26], 18, 113, col)
...
def mostrar_normal(node_id, lec, hora="", conectado=False, vista=0,
                   status=None, status_col=None):
    """Pantalla normal con vista rotativa (0=ambiente, 1=aire, 2=movimiento).
    Si se pasa 'status', la barra inferior lo muestra (p.ej. 'Midiendo...',
    'Enviando FB...', 'Enviado') con su color."""
    if not tft:
        return
    try:
        tft.fill(NEGRO)
        ...
        if status:
            _barra(status, status_col or VERDE)
        elif conectado:
